    ...
    var body: some View {
        if clothes.isEmpty {
            ...
            ForEach(clothes) { clothing in
                ClothingRowView(
                    clothing: clothing,
                    isSelected: selectedClothing?.id == clothing.id, // unwrap and compare
                    onSelect: {
...
struct ClothingList: View {
    ...
    var body: some View {
        ForEach(clothes, id: \.id) { clothing in
            if isCreatingOutfit {
                // Use Button for selecting in outfit creation context
                Button(action: {
                    selectedClothing = clothing
                    // Add logic here to handle outfit creation (e.g., adding to outfit store)
                }) {
                    ClothingListRow(clothing: clothing)
                }
            } else {
                // Use NavigationLink for browsing context
                NavigationLink(destination: ClothingInfoView(clothing: clothing)) {
                    ClothingListRow(clothing: clothing)
                }
            }
